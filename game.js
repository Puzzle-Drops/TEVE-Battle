// Game Core
		...
		this.currentGearFilter = 'trinket'; // Add this line after other properties
		this.currentStashFilter = null; // Add this line to track stash page filter
...
filterStashSlots(source) {
    ...
    if (source === 'gear') {
        ...
        if (select) {
            this.currentStashFilter = select.value;
        }
...
init() {
    ...
    // Equip the first 3 villagers (not the tester) with starter gear
	/*
    const villagerItems = ['villager_amulet', 'villager_sword', 'villager_shield', 
                          'villager_helmet', 'villager_chestplate', 'villager_boots'];

for (let i = 1; i <= 3; i++) {
        const hero = this.heroes[i+5];
        
        // Create and equip each villager item
        villagerItems.forEach(itemId => {
            const item = new Item(itemId);
            // Set quality to 1/5 for all stats
            item.setFixedQuality(1);
            // Mark as refined so it can't be improved
            item.refined = true;
            
            // Equip the item to the appropriate slot
            hero.equipItem(item, item.slot);
        });
        
        console.log(`Equipped villager ${i+5} (${hero.name}) with starter gear`);
    }
*/
...
showItemOptions(item, itemIndex, family, isEquipped = false, slot = null) {
    ...
    } else if (this.currentScreen === 'heroesScreen' && this.selectedHero !== undefined) {
        options.push({
            text: 'Equip',
            action: () => {
...
options.push({
    ...
    action: () => {
        if (isEquipped) {
            // Unequip first
            this.heroes[this.selectedHero].unequipItem(slot);
        } else {
            // Remove from stash
